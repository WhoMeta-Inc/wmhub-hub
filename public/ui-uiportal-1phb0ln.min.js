...
Component: j-Approve
Version: 1
Updated: 2024-01-11T20:17:00.000Z
Author: Peter Širka


Component: j-Box
Version: 1
Updated: 2024-02-11T19:39:00.000Z
Author: Peter Širka


Component: j-ColorSelector
Version: 1
Updated: 2023-01-23T11:01:00.000Z
Author: Peter Širka


Component: j-DatePicker
Version: 1
Updated: 2024-01-18T11:20:00.000Z
Author: Peter Širka


Component: j-Detach
Version: 1
Updated: 2023-01-25T10:20:00.000Z
Author: Peter Širka
...
Component: j-Exec
Version: 1
Updated: 2024-01-10T16:20:00.000Z
Author: Peter Širka


Component: j-Features
Version: 1
Updated: 2023-01-31T09:04:33.000Z
Author: Peter Širka
...
Component: j-Input
Version: 1
Updated: 2024-01-10T15:59:00.000Z
Author: Peter Širka


Component: Intranet CSS
Version: 5
Updated: 2023-03-23T23:29:02.000Z
Author: Peter Širka
...
Component: j-Menu
Version: 1
Updated: 2024-01-05T10:11:00.000Z
Author: Peter Širka


Component: j-Message
Version: 1
Updated: 2024-01-30T11:46:01.000Z
Author: Peter Širka


Component: j-MiniForm
Version: 2
Updated: 2024-02-11T19:37:00.000Z
Author: Peter Širka
...
Component: j-Preview
Version: 1
Updated: 2024-03-15T11:18:00.000Z
Author: Peter Širka


Component: j-Rating
Version: 1
Updated: 2023-02-01T10:30:00.000Z
Author: Peter Širka
...
Component: j-Sounds
Version: 1
Updated: 2023-01-31T12:17:03.000Z
Author: Peter Širka


Component: j-TableGrid
Version: 1
Updated: 2023-01-31T11:41:00.000Z
Author: Peter Širka


Component: Tangular-Initials
Version: 1
Updated: 2023-02-19T22:28:00.000Z
Author: Peter Širka


Component: j-UIBuilder
Version: 1
Updated: 2023-11-13T15:42:01.000Z
Author: Peter Širka


Component: j-UIStudio
Version: 1
Updated: 2024-01-16T23:26:01.000Z
Author: Peter Širka
...
.ui-colorselector{margin:0;padding:0;list-style-type:none}.ui-colorselector li{position:relative;display:inline-block;width:10%;margin:10px 0;height:20px;vertical-align:middle;cursor:pointer}.ui-colorselector li:first-child{border-top-left-radius:var(--radius);border-bottom-left-radius:var(--radius)}.ui-colorselector li:last-child{border-top-right-radius:var(--radius);border-bottom-right-radius:var(--radius)}.ui-colorselector .selected{margin:0;height:30px}.ui-colorselector-label{font-size:12px}.ui-colorselector.ui-disabled li{cursor:not-allowed;opacity:0.9}
...
COMPONENT('colorselector','colors:#DA4453,#E9573F,#F6BB42,#119CBC,#37BC9B,#3BAFDA,#119CBC,#967ADC,#D770AD,#656D7D;empty:true',function(self,config){var selected,list,content,colors=null;self.nocompile&&self.nocompile();self.validate=function(value){return config.disabled||!config.required?true:colors.indexOf(value)===-1};self.configure=function(key,value,init){if(init)return;var redraw=false;switch(key){case'required':self.find('.ui-colorselector-label').tclas('.ui-colorselector-required',value);break;case'disabled':self.tclass('ui-disabled',value);break;case'colors':colors=value.split(',').trim();break;case'label':case'icon':redraw=true;break}redraw&&setTimeout2('redraw.'+self.id,function(){self.redraw();self.refresh()},100)};self.redraw=function(){var builder=[],label=config.label||content;label&&builder.push('<div class="ui-colorselector-label{1}">{2}{0}</div>'.format(label,config.required?' ui-colorselector-required':'',config.icon?'<i class="ti ti-{0}"></i>'.format(config.icon):''));builder.push('<ul class="ui-colorselector">');for(var i=0,length=colors.length;i<length;i++){var color=colors[i];color&&builder.push('<li data-index="{0}" style="background-color:{1}"></li>'.format(i,color))}builder.push('</ul>');self.html(builder.join(''));self.tclass('ui-disabled',config.disabled);list=self.find('li')};self.make=function(){colors=config.colors.split(',').trim();self.redraw();self.event('click','li',function(){if(config.disabled)return;var color=colors[+this.getAttribute('data-index')];if(!config.required&&color===self.get())color='';self.change(true);self.set(color)})};self.setter=function(value){var index=colors.indexOf(value);selected&&selected.rclass('selected');if(index!==-1){selected=list.eq(index);selected.aclass('selected')}}});
...
COMPONENT('detach',function(self,config){self.readonly();var children=[],detached=false;self.destroy=function(){config.remove&&self.EXEC(config.remove,children,self.element)};self.setter=function(value){if(value===true){if(!detached){detached=true;children.length=0;for(var i=0;i<self.dom.children.length;i++)children.push(self.dom.children[i]);self.EXEC(config.detach,children,self.element)}}else{if(detached){self.EXEC(config.attach,children,self.element);detached=false;children.length=0}}}});
...
COMPONENT('tablegrid','count:3;fill:0;scrollbar:1;visibleY:1;margin:0;row:0;scrolltop:1',function(self,config,cls){var cls2='.'+cls,empty='',container,area,display,isinit=false;self.readonly();self.make=function(){self.aclass(cls);var templates=self.find('scri'+'pt');for(var i=0;i<templates.length;i++){var html=templates[i].innerHTML;if(i)empty=html;else self.template=Tangular.compile(html)}self.html('<div class="{0}-scrollbar"><div class="{0}-container"></div></div>'.format(cls));container=self.find(cls2+'-container');area=self.find(cls2+'-scrollbar');self.scrollbar=config.scrollbar&&config.parent?SCROLLBAR(area,{shadow:config.scrollbarshadow,visibleY:config.visibleY,orientation:'y'}):null;self.on('resize + resize2',self.resize);isinit=true;self.resizeforce();isinit=false};self.resize=function(){setTimeout2(self.ID,self.resizeforce,300)};self.resizeforce=function(){if(!config.parent)return;var parent=self.parent(config.parent);if(self.scrollbar){area.css({height:parent.height()-config.margin});setTimeout(self.scrollbar.resize,10)}display=WIDTH(parent);if(!isinit)self.refresh()};self.setter=function(value){if(!value){container.empty();return}var parent=config.parent?self.parent(config.parent):null;if(display==null)display=config.parent?WIDTH(parent):WIDTH();var rows=[],cols=[],count=config['count'+display]||config.count,items=value.slice(0);var width=(100/count).floor(2);var rowheight=config['row'+display]||config.row,noscroll=false;while(items.length%count)items.push(null);self.scrollbar&&self.scrollbar.resize();if(config.fill&&config.scrollbar){var diff=Math.ceil(area.height()/rowheight);var rowcount=items.length/count;for(var i=rowcount;i<diff;i++){noscroll=true;for(var j=0;j<count;j++)items.push(null)}}var cssheight=rowheight?';height:{0}px'.format(rowheight):'';for(var i=0;i<items.length;i++){var row=i%count,item=items[i],c=[cls+'-item'];if(!cols.length)c.push(cls+'-first');if(item==null)c.push(cls+'-empty');if(rows.length)c.push(cls+'-bt');if(cols.length)c.push(cls+'-bl');if(row===count-1)c.push(cls+'-last');cols.push('<div class="{0}" style="width:{1}%{3}">{2}</div>'.format(c.join(' '),width,item?self.template({value:item}):empty,cssheight));if(row===count-1){rows.push('<div><div class="{0}-row">{1}</div><div class="clearfix"></div></div>'.format(cls,cols.join('')));cols.length=0}}if(cols.length)rows.push('<div><div class="{0}-row">{1}</div><div class="clearfix"></div></div>'.format(cls,cols.join('')));var html=rows.join('');container.html(html);if(config.scrollbar){area.tclass('ui-scrollbar-noscroll',noscroll);config.scrolltop&&self.scrollbar&&self.scrollbar.scrollTop(0)}html.COMPILABLE()&&COMPILE(container)}});
