...
Version: 1
Updated: 2023-06-13T15:52:00.000Z
Author: Peter Širka


Component: j-Box
Version: 1
Updated: 2024-06-10T13:42:00.000Z
Author: Peter Širka
...
Version: 1
Updated: 2024-05-02T15:41:00.000Z
Author: Denis Granec
...
Version: 2
Updated: 2024-06-14T07:50:00.001Z
Author: Peter Širka


Component: j-DatePicker
Version: 1
Updated: 2024-05-02T15:40:02.000Z
Author: Peter Širka


Component: j-Directory
Version: 1
Updated: 2024-05-02T15:35:02.000Z
Author: Peter Širka


Component: j-Edit
Version: 1
Updated: 2024-03-17T15:49:00.000Z
Author: Peter Širka
...
Component: j-Exec
Version: 1
Updated: 2024-06-10T13:44:00.000Z
Author: Peter Širka


Component: j-Icons
Version: 1
Updated: 2023-03-07T20:40:01.000Z
Author: Peter Širka
...
Component: j-Input
Version: 1
Updated: 2024-06-10T13:36:00.000Z
Author: Peter Širka
...
Version: 1
Updated: 2023-02-01T08:34:01.000Z
Author: Peter Širka


Component: j-Notify
Version: 1
Updated: 2023-03-11T19:50:01.000Z
Author: Peter Širka
...
Component: j-Prompt
Version: 1
Updated: 2024-04-09T21:06:01.000Z
Author: Peter Širka


Component: j-Selection
Version: 1
Updated: 2023-02-03T20:40:01.000Z
Author: Peter Širka
...
Version: 1
Updated: 2024-06-12T10:38:00.000Z
Author: Peter Širka


Component: j-ViewBox
Version: 1
Updated: 2024-06-10T13:58:00.000Z
Author: Peter Širka
*/
...
COMPONENT('validate','delay:100;flags:visible;changes:0;strictchanges:0',function(self,config,cls){var elements=null,def='button[name="submit"]',flags=null,tracked=false,reset=0,old,track,currentvalue,customvalidation=null,currentpath='';self.readonly();self.make=function(){elements=self.find(config.selector||def);currentpath=self.path.toString()};self.configure=function(key,value,init){switch(key){case'selector':if(!init)elements=self.find(value||def);break;case'flags':if(value){flags=value.split(',');for(var i=0;i<flags.length;i++)flags[i]='@'+flags[i];flags=flags.join(' ')}else flags=null;break;case'track':track=value.split(',').trim();break;case'if':customvalidation=new Function('value','path','return '+value);break}};var settracked=function(){tracked=0};var backup=null,current=null;self.setter=function(value,path,type){currentvalue=value;var is=path===currentpath||path.length<currentpath.length;if(config.changes){current=STRINGIFY(value,config.strictchanges!=true);if(is)backup=current;else is=backup===current}if(reset!==is){reset=is;self.tclass(cls+'-modified',!reset)}if((type===1||type===2)&&track&&track.length){for(var i=0;i<track.length;i++){if(path.indexOf(track[i])!==-1){tracked=1;return}}if(tracked===1){tracked=2;setTimeout(settracked,config.delay*3)}}};var check=function(){var arr=COMPONENTS(self.path+(flags?(' '+flags):''));var disabled=false,modified=false;for(var m of arr){if(config.validonly){if(m.config.invalid){disabled=true;break}}else if(m.config.invalid){disabled=false;break}else if(m.config.modified)modified=true}if(!disabled){if(!config.validonly)disabled=modified==false}if(!disabled&&config.if)disabled=customvalidation(self.get(),'');if(!disabled&&config.changes&&backup===current)disabled=true;if(HIDDEN(self.element))disabled=true;if(disabled!==old){elements.prop('disabled',disabled);self.tclass(cls+'-ok',!disabled);self.tclass(cls+'-no',disabled);old=disabled;if(!old&&config.exec)self.SEEX(config.exec,currentvalue);config.output&&self.SEEX(config.output,!disabled)}};self.state2=function(){setTimeout2(self.ID,check,config.delay)};self.state=function(type,what){var isreset=jComponent.is20?(!config.modified&&!config.touched):(type===3||what===3);if(isreset){self.rclass(cls+'-modified');tracked=0;backup=current}setTimeout2(self.ID,check,config.delay)}});
